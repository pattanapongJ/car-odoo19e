...
export class CarConfigurator extends Interaction {
    ...
    /* Restrict the interior options to those allowed by the selected exterior
       colour (its "Available Interiors"). Disabled interiors are greyed out and,
       if the current selection becomes unavailable, the first allowed interior
       is selected instead. Unpublished options stay disabled regardless. */
    _refreshInteriorAvailability() {
        ...
        for (const input of interiorInputs) {
            const unpublished = input.dataset.unavailable === "1";
            const ok = !unpublished && (allowed === null || allowed.has(input.dataset.valueId));
            input.disabled = !ok;
            const label = input.closest("label");
            if (label) {
